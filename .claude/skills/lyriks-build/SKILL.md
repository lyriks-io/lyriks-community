...
- **Architecture progresses from logical design to implementation decisions.** Read `describe_section(architecture)` for the deployed schema. On versions supporting `stage`, start new designs at `logical`: named components, responsibilities, boundaries and constraints, without choosing technologies just to fill layers. Record concrete choices, versions, alternatives and rationale in Documents & Sources; cite them with `referenceDocId` and `sourceIds`. Mark decision sources proposed, accepted or superseded where supported. Move to `implementation` when making stack choices. Existing populated boards keep their existing choices. Never infer an accepted stack from a logical component name or invent a documentation URL. Older deployments may not support these fields: preserve logical intent in source notes and report the version limitation rather than silently dropping it. `referenceDocs` is migration-only; use the shared evidence register.
...
Once the spec is authored and validated, the work that FOLLOWS from it (tickets, stories, sprints, and the re-sync when code lands) is governed by `lyriks-delivery`. Load it instead of improvising a convention, and never copy modeled behavior into a tracker: the spec has one home. The conversation stays bound to the project after that: every later change to what the product does is a spec change first, then code, then index (the binding section of `lyriks-delivery`), and every question about its behavior is a spec read first, whether or not the user names Lyriks again.
...
- `set_section` needs the WHOLE section document, and on the Experience it is off-limits once screens exist: `build_screen` per screen, `patch_section` for everything else (`builder.theme`, `screens`, `journeys`), never a resend of `builder.nodes`. A whole-Experience replacement is refused by agent runtimes' safety controls; per-screen writes never trigger them.
...
Experience validates interaction, navigation, input validation, guards and state in a model. It remains useful for applications and for game menus and interactions. Keep it; do not describe a passing prototype scenario as proof of the real renderer, collisions, networking, visual quality or load capacity. Record real test results separately in source evidence where supported, with the tested build, command, artifact, provenance and criterion IDs. Report unrun and blocked checks honestly. Source notes remain the fallback on older deployments.
