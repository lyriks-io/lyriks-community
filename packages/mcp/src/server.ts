...
// Whenever a client tells a person that something waits on them, the person
// gets there in one click: every place a person acts on carries its own link,
// made whole by the server that knows which installation it serves.
const PERSON_LINKS =
  'LINKS FOR THE PERSON: every place a person acts on carries `href`, an absolute link to the page that opens ON that place: ' +
  'each request card, the dossier itself, `proposals.href` (the first proposal waiting), `gate.href` (the next gate, to cross or waive), ' +
  'every proposal, every field row, every report line, every observation, and every refused result a person resolves. ' +
  'Whenever you tell the person that something waits on them (a signature, an open question, a gate, a line to decide, a remark to rule), ' +
  'hand them that href as a clickable markdown link, one per thing, instead of describing where to click; never build a page address yourself.'

// The one rule about Evolution, spelled once. The instructions, both Evolution
// tools, the binding block and the per-prompt hook carry this same text, so a
// client that reads two of them never finds two different rules. Evolution used
// to be the mandatory door for every change; field use showed a dossier for a
// two-line fix cost more than the fix, so agents skipped it anyway. It is now a
// tool the person reaches for, not a toll on every change.
export const EVOLUTION_IS_OPTIONAL =
  'Evolution is OPTIONAL. A change to an existing product is made directly: the spec first through ' +
  'the section tools (apply_behavior_batch, patch_section, build_screen, wire_element), then the code, ' +
  'then sync_implementation_index. Open an Evolution request (get_evolution / apply_evolution_batch, ' +
  'the lyriks-evolution skill) only when the person asks for one, or asks for a change to be QUALIFIED ' +
  'before anyone decides it (what it would involve, an impact report, an estimate, a decision that ' +
  'belongs to someone else): the dossier then carries the change as a draft (add_draft_leaf), reads its ' +
  'impact and coherence on the product as it would be, and its freeze into Verify writes the sections. ' +
  'Never open a dossier on your own for an ordinary change, and never hold a change back because no ' +
  'dossier exists. A change the person is already carrying in an open request continues in that request.'
...
export function createMcpServer(ee: BoundOverlay | null, lyriksToken?: string): McpServer {
  const mcp = new McpServer(
    { name: 'lyriks', version: '1.0.0' },
    {
      ...
      instructions:
        ...
        'lyriks-evolution when the person wants a change qualified or carried through a dossier. ' +
        EVOLUTION_IS_OPTIONAL + ' ' +
        ...
        'a flow, a message) is a SPEC CHANGE FIRST: author it in the bound project through the section ' +
        'tools (apply_behavior_batch, patch_section, build_screen, wire_element), then write the code, then ' +
        'record where the code implements it (index entries carrying their signature line, ' +
        'sync_implementation_index; lyriks-delivery step 4). A request that asks ' +
  ...
  mcp.tool(
    ...
    'Reconcile locally installed Lyriks authoring skills with this server. Works for ANY agent runtime, not just Claude. Set `client` to what you are ("claude" | "codex" | "gemini" | "copilot" | "generic") so the response carries your layout alone; omit it and every layout comes back for you to choose from. First read the `contentHash:` frontmatter line of each installed SKILL.md you have, under `.claude/skills/<id>/` or `.agents/skills/<id>/` (missing file or missing line = omit the hash). Pass them as `installed`; omit or send [] on a fresh machine — everything then comes back as "new". Response: { skills: [{ id, name, description, contentHash, status: "up-to-date"|"update"|"new", installPath, installTargets: [{ client, label, path, pointerPath?, pointerBlock? }], installContent? }], unknown: [ids] }. For each entry that carries `installContent`, take the `installTargets` item matching your runtime and: (1) write `installContent` VERBATIM to `path` (create directories as needed); (2) when the target also has `pointerPath`, merge `pointerBlock` into that file — replace the region between its `<!-- lyriks-skill:<id> -->` and `<!-- /lyriks-skill:<id> -->` markers if present, otherwise append the block, and change NOTHING else in that file (it belongs to the user). An entry is current only when status is "up-to-date". With include_content:false, new/update entries carry contentDeferred:true; fetch their complete content before installing or following them. Use skill_ids to synchronize only the guides relevant to the task. SIZE: one answer carries whole guides while they fit its content budget, and the ones past it come back with contentDeferred:true under `deferredForSize` (fetch each with get_skill, or call sync_skills again naming it in skill_ids); a guide is never cut in half, since an install writes it verbatim. Ids in `unknown` are not published by this server — leave those local skills alone. The response also carries `binding`: what keeps this repository bound to its Lyriks project in every later session, for every user. Take the `binding.targets` item matching your runtime and: (3) merge its `pointerBlock` into `pointerPath` (the instruction file you always load: CLAUDE.md, AGENTS.md, GEMINI.md or .github/copilot-instructions.md), replacing the region between `<!-- lyriks-binding -->` and `<!-- /lyriks-binding -->` if present, appending the block otherwise, touching nothing else; (4) when the target carries `hook` (Claude Code), write `hook.content` VERBATIM to `hook.path` and wire it: if `hook.settingsPath` does not exist write `hook.settingsContent` as is, otherwise add `hook.settingsEntry` under `hooks.UserPromptSubmit[0].hooks` unless a command naming `hook.path` is already there. Then, whatever your runtime: (5) when `binding.tools` is present (the top level of `binding`, the same list for every runtime; an older platform sends none), for each entry write `content` VERBATIM to `path` (create directories as needed), every one of them, side by side under `.lyriks/tools/`, since they import each other. They are plain Node helper scripts for this repository: syncing an index or applying a batch too large to type as a tool argument, and checking the index by signature text instead of by exact line; the `purpose` of each entry says what it does and how to run it. An entry (or the `hook`) that comes back with `unchanged: true` has no `content`: you already hold that exact version, write nothing. To get that, report what you hold in `installed_tools`: for each file under `.lyriks/tools/` and for `.claude/hooks/lyriks-bound-prompt.mjs`, its `path` and the hash its `// contentHash: <hash>` line carries (the first line, or the second after `#!`); a file without that line is reported without a hash, or left out, and comes back whole. Omit `installed_tools` and every file comes back with its content. Pass `project_id` (the wizard project slug this repository is specified in) as soon as you know it, so the block names the project; a project created later gets one more sync_skills call with its id. Call this at the START of any session that will author Lyriks project data, before following a skill.',
    {
      ...
      installed_tools: z
        .array(z.object({
          path: z.string().describe('The installed file, relative to the workspace root: .lyriks/tools/<file>.mjs, or .claude/hooks/lyriks-bound-prompt.mjs'),
          content_hash: z.string().optional().describe('The value of its `// contentHash:` line; omit when the file has none'),
        }))
        .optional()
        .describe('The binding helper scripts and hook already installed; a file reported with the published hash comes back `unchanged: true` without content. Omit to receive every file whole'),
      client: z
  ...
  mcp.tool(
    ...
    'The Evolution section as ONE aggregate: the board (the requests FIRST, one card per live change request: stage, the stage its own state supports, maturity, open questions, pending proposals, coherence delta, blocking findings, undecided report lines, acceptance debt, waiver) or, with request_id, ONE dossier IN COUNTS: its touched features, how many fields are filled, empty, critical and empty, open questions, awaiting a proposal or discussed, how many proposals wait and how many are flagged or blocked, how many readings are filled and how many move, the maturity per block with the critical holes named, the coherence findings the engine published, the impact in short (one plain-language line per plane, the spec plane and the code plane, how much moves per section, plus the direct hits), the next gate and exactly why it refuses, the report counts, the observations, the acceptance debt and the last timeline entries. That answer is the SAME SIZE whether the request touches one feature or twenty, because every list it counts comes ONE PART at a time, narrowed to one touched feature with `leaf` and paged with `offset`/`limit`: part:"fields" (one row per inline field per touched feature: the value the owning section holds, whether it is filled, the open question, the proposal waiting, the thread), part:"proposals" (every pending proposal with its full value, reasoning, sources and whether a person may accept it as it stands), part:"impact" (every impacted node under one hypothesis, the one that ran last by default and any other with `hypothesis` add|change|remove, since each run keeps its own findings, `section` narrows to one of leaves|screens_and_journeys|entities_and_fields|rules_and_scenarios|permissions|glossary_terms|code; the code section lists the files the implementation index anchors on the touched and reached features), part:"report" (every line of the current iteration, `verdict` narrows to conform|non_conform|missing|out_of_scope|regression; each line carries `scope`: "request" for an element this request brought, "inherited" for one the touched features already held at the first freeze, which is their backlog and never holds the report open; `undecided` counts the request lines only, `inherited` counts the rest; `scope` narrows the list), part:"readings" (the blocks edited elsewhere: behaviour, grants, entities, dependencies; an empty reading = author it in the owning section and it fills itself), part:"drafts" (what the request PROPOSES, in full: each drafted feature with its kind add|amend|remove, the leaf it stands for, its name, description, the four why-fields, its acceptance criteria, dependencies, sources and behaviour rows, plus the leaf id it became once the freeze wrote it), part:"history". Read the counts first, then the one list you need, one feature at a time. A list page that would not fit the result cap comes back shorter in the SAME shape, with `nextOffset`. The board counts leaves and sources; read them with part:"leaves" or part:"sources". A request is named by its full id or any unique prefix. Read it when the person wants a change qualified or carried through a dossier, or to see the requests already open. ' + EVOLUTION_IS_OPTIONAL + ' The aggregate is precomputed server-side, and `fieldsAvailable` lists the field paths apply_evolution_batch can propose on; `originsAvailable` lists the six origin codes open_request takes; `members` is the workspace roster a proposal can be handed to (empty where one member is alone). Read this instead of get_section(evolution), which is the raw document without any of the derived readings. ' + PERSON_LINKS,
    {
      ...
      scope: z.enum(['request', 'inherited']).optional().describe('part:"report": "request" keeps the lines on elements this request brought (they hold the report open), "inherited" the lines on elements the touched features already held at the first freeze (their backlog: shown, never a gate)'),
      ...
      offset: z.number().int().nonnegative().optional().describe('page start for any list part (the answer says total, matched, offset, limit, and nextOffset when rows remain)'),
      limit: z.number().int().positive().optional().describe('page size for any list part (default 150); a page that would not fit the result cap comes back shorter, same shape, with nextOffset and pageNote'),
    },
  ...
  mcp.tool(
    ...
    'Drive a change request through its lifecycle with typed operations, applied ATOMICALLY by the platform under the same guards as the dossier page: nothing lands unless every op is allowed, and a refusal names the op and the sentence the spec wrote. WHEN TO OPEN A REQUEST: ' + EVOLUTION_IS_OPTIONAL + ' Each operation is `{ op, ... }` (`kind` is accepted as the same thing, since the behavior batch spells it that way). YOU (the AI client) may: open_request {title, origin, requester?, leafIds[] = EXISTING leaf features the change touches}; every requestId accepts the full id or any unique prefix of 4+ characters; the six origins are internal_idea|customer_feedback|support_ticket|market_watch|regulatory|technical_debt, and get_evolution lists them as originsAvailable, update_request {requestId, title?, origin?, requester?}, set_leaves {requestId, leafIds[] = leaves that EXIST, or drafts this request carries}, add_draft_leaf {requestId, kind add|amend|remove (default add), baseLeafId (required for amend and remove: the EXISTING leaf it stands for), name (required for add), description?, coreId?, parentFamilyId?, objective?, problem?, expectedEffect?, value?, acceptanceCriteria[]?, dependsOn[]?, sourceIds[]?, behaviour[]? (each {kind surface|state|action|rule|scenario, name, detail?})} = PUT THE CHANGE ON THE DOSSIER. This is how a capability the product does not have yet gets qualified: the draft is counted among the touched features straight away, the impact walk starts FROM it instead of from the hole where it would sit, the coherence check catches what it would contradict, and NOTHING is written into any section until the freeze. AN AMENDMENT IS A DELTA: on kind "amend", acceptanceCriteria[] are the criteria it ADDS (the feature\'s other criteria stay), retireCriteria[] names by id the feature\'s criteria it retires, changeCriteria[] ({id, text}) rewords one in place under its id, descriptionPatch ({find, replace}, or a list of them; `find` must occur exactly once in the feature\'s description) and descriptionAppend edit the description without resending it (a `description` replaces it outright); every id and passage is checked against the feature when the draft is made. Values typed on an amend draft (objective, problem, value, criteria...) count as filled for the feature it amends, so do not propose them again. update_draft_leaf {requestId, draftId, ...the same fields} (only what you name is touched), remove_draft_leaf {requestId, draftId} (the draft and what was proposed on it go; the specification is exactly as it was), propose {requestId, fieldPath (from get_evolution fieldsAvailable), leafId, value (a string, or an array of lines for a field whose kind is "list"), whatWasRead (what the sources say), whatWasInferred (what you concluded from it), citedSourceIds[] (documents register ids: register the source first, in the same batch with register_source, or with patch_section(documents), NOT attach_source)}. The two halves are named fields, judged on being filled, in the language the project is written in: nothing is looked for in your wording, and a proposal missing either half is REFUSED when it is proposed (a batch then stops there, so put proposals before decisions), register_source {title, url? (a web address anyone can open), note? (the words themselves, verbatim, when there is no such address), kind?, id?} (writes a row of the documents register; one of url or note is required; answers the id to cite; needs no requestId), withdraw_proposal {requestId, proposalId} (take back a proposal YOU made while nobody has decided it, so a wrong or flagged value never holds a field nobody will sign; a person refuses instead), post_on_field {requestId, fieldPath, leafId?, body}, run_impact {requestId, hypothesis add|change|remove, depth 1..5} (COMPUTED: the spec plane over the knowledge graph, the code plane over the synced implementation index; run the hypotheses the request asks for, one per kind of draft it carries, in one batch: each row reads with the verb of the draft behind the feature it was reached from; remove on a request that removes nothing is the cost of taking the change back out, a reading of its own run only when the person asks; an empty report says why in impact.emptyBecause; sync_implementation_index first for a real code plane), run_coherence {requestId} (COMPUTED by the coherence engine over the whole project), build_implementation_report {requestId} (DERIVED from the synced implementation index against the frozen version: sync_implementation_index first, in Verify; lines on elements the touched features already held at the first freeze come back scope "inherited" and do not hold the report open, so only this request\'s own lines need a decision before Accept). A PERSON decides, and you relay their decision ONLY when they told you to, with as_person:true on the batch (the act lands as theirs, channel stamped on the timeline): decide_proposal {requestId, proposalId, decision accept|refuse|reword, comment?, value?, sense?} (accept WRITES the value into the owning section; a glossary flag (flaggedWords, each naming the agreed term) WARNS and never blocks, and sense records the person\'s words when they keep a flagged wording in another sense; on a proposal with tagged reviewers, accept is that reviewer\'s validation and the value is written once every reviewer validated, refuse is an invalidation that refuses it), tag_reviewers {requestId, proposalId, reviewerIds[]} (hand a proposal to named members of the workspace, ids = emails from get_evolution `members`; only where the roster holds more than one member, Enterprise), mark_open_question / answer_open_question {requestId, fieldPath, leafId?}, cross_stage {requestId, waiverReason?} (one gate at a time, and Specify does not close while a proposal still awaits a decision, since the maturity next to the dossier is inherited from the features the change touches and never proves the change itself was specified: Specify > Challenge > Verify (freezes the spec as a numbered version) > Accept > Delivered; a waiver crosses an unmet gate with a stated reason), lift_waiver, rebrief (back to Specify, amends a frozen spec), decide_line {requestId, lineId | verdict, decision validated|invalidated|adopted|removed}, rule_observation {requestId, observationId, ruling validated|invalidated|deferred|requalified, reason?}, fold_back {requestId, observationId, leafId, text} (writes an acceptance criterion on the feature), close_request, delete_request. AN OBSERVATION IS LOGGED BY THE PERSON WALKING THE PRODUCT, on the dossier page, because it is anchored on the screen and the element they are looking at and carries their annotated capture: you rule on the observations get_evolution lists and fold the validated ones back, and there is no operation to log one in their place. An observationId the dossier does not list is refused, which is that rule, not a missing feature. INSIDE A DOSSIER, NOTHING REACHES A SECTION BEFORE THE FREEZE: the change a request carries is not also written directly (other changes are, since Evolution is optional). A feature the change would add is an add_draft_leaf on the dossier, and crossing into Verify is what writes every draft into the features section, stamped with the request. Other predicted new things are impact findings; spec values are proposals. The answer carries each op result and the touched requests as CARDS; read the dossier again with get_evolution. Order of work: open_request > add_draft_leaf (what the change IS) and/or set_leaves (what it touches) > run_impact (the hypotheses the drafts ask for) > run_coherence > propose on the open questions and empty critical fields > the person decides > cross_stage. A request that disturbs nothing and contradicts nothing needs none of the last three: it crosses its own gates and closes itself the moment the readings come back, and the answer says so. ' + PERSON_LINKS + ' Follow the lyriks-evolution skill.',
    {
      project_id: z.string().describe('Lyriks project id'),
