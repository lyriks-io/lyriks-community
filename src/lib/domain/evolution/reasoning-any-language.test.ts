...
describe('a reasoning is judged on its two fields, in any language', () => {
	...
	it('does not reward English words typed into a single free text', () => {
		const proposal = proposalOf(
			propose({ reasoning: 'I read the brief and I infer the rest from it.' })
		);
		expect(proposal.reasoningSeparatesReadFromInferred).toBe(false);
	});

	it('refuses a lone free text by naming the two fields to fill', () => {
		const proposal = proposalOf(propose({ reasoning: 'Lu dans le brief, puis deduit.' }));
		const verdict = canAcceptProposal(person, proposal);
		expect(verdict.ok).toBe(false);
		expect(verdict.ok === false && verdict.reason).toContain('whatWasRead');
		expect(verdict.ok === false && verdict.reason).toContain('whatWasInferred');
	});
