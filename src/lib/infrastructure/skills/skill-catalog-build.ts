...
export function buildBindingBlock(projectId?: string | null): string {
	...
	return [
		...
		'',
		'- A change to what the product does (a capability, a rule, a screen, a field, a state, a flow, a message) opens an EVOLUTION REQUEST first (`get_evolution` / `apply_evolution_batch`, skill `lyriks-evolution`): the dossier carries the change as a draft (`add_draft_leaf`), the impact and coherence reports read the product as it would be, and the freeze is what writes the sections. Then the code, then the index entries carrying their signature line and `sync_implementation_index`. The sync takes a partial index: send the entries of the features you touched, each action or surface with all of its children, never the whole file when it is large. Never let the code get ahead of the spec; a number only a measurement can settle is specified as a provisional constant, measured, then written once.',
		'- There is NO threshold and nothing for you to judge. Never decide a change is too small for a dossier: a request that disturbs nothing and contradicts nothing crosses its own gates and closes itself in the same act, so it costs its author nothing but the trace it leaves. Who signs follows the roster, not the size of the change: alone in the workspace you carry the request through yourself, proposals included; from two members up a person signs each one.',
		'- The same door serves a change someone only asks to QUALIFY (what it would involve, an estimate, an impact report, a decision that belongs to someone else): it is the same dossier, left short of the freeze. Nothing reaches a section before that freeze, so there is no longer anything to ask about: open the request either way.',
		'- A project still being written for the FIRST time is not a change to an existing product; `lyriks-build` authors it directly. The door binds a project once its specification has been declared finished at least once, or once code is anchored to it.',
